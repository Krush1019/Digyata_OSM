	...
	<div class="navbar-header">
		<ul class="nav navbar-nav flex-row">
			<li class="nav-item mr-auto"><a class="navbar-brand" href="{{route('admin-dashboard')}}">
				...
				<h2 class="brand-text mb-0">Digyata</h2> --}}
			</a></li>
						...
						<li class="nav-item {{ (request()->is($menu->url)) ? 'active' : '' }} {{ $custom_classes }}">
							<a href="{{ $menu->url }}">
								...
								@if (isset($menu->badge))
									...
									@if ( $menu->url == "service-manage")
										@isset($countAdminServiceList)
											<span class="{{ isset($menu->badgeClass) ? $menu->badgeClass.' test' : $badgeClasses.' notTest' }} "> {{$countAdminServiceList}}</span>
										@endisset
									@else
